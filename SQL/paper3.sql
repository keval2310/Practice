...
select
	p.ProductName,
	count(distinct o.CustomerID) Customer_count
from Products p
left join Orders o
...
group by 
	c.Name
having
	year(min(o.OrderDate)) > 2023 and
	year(min(o.OrderDate)) < 2025
...
group by 
	p.ProductName
order by
	sum(o.TotalOrderAmount) desc

--Question 5: Find the latest order placed for each customer.

select
	c.Name CustomerName,
	max(o.OrderDate) LatestOrderDate
from Customers c
join Orders o
on c.CustomerID = o.CustomerID
group by 
...
select
	c.Name,
	count(distinct p.Category) UniqueCategoriesCount
from Customers c
join Orders o
on c.CustomerID = o.CustomerID
join Products p
...
group by
	p.ProductName
having
	DATEDIFF(MONTH, getdate(), max(o.OrderDate)) < 6
